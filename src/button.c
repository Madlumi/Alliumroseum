...
typedef struct _BTN_LIST {
   ...
   V (*tick)(struct _BTN_LIST *self, POINT mp);
} btn_list;

btn *btn_new(SDL_Rect r, TEXTURE * tx_norm,TEXTURE * tx_high, TEXTURE * tx_click,TEXTURE * tx_dis){
...
static void _draw(btn_list * b, SDL_Renderer *ren){FOR(b->i,{
   switch (b->btns[i].st) {
      case NORM:
         SDL_RenderCopy(ren, b->btns[i].tx_norm, NULL, &b->btns[i].r);break;
      case HOVER:
         SDL_RenderCopy(ren, b->btns[i].tx_high, NULL, &b->btns[i].r);break;
      case DISABLED:
         SDL_RenderCopy(ren, b->btns[i].tx_dis, NULL, &b->btns[i].r);break;
      case CLICK:
         SDL_RenderCopy(ren, b->btns[i].tx_click, NULL, &b->btns[i].r);break;
   }

})}
static void _tick(btn_list * b, POINT mp){FOR(b->i,{ if(SDL_PointInRect(&mp, &b->btns[i].r)){b->btns[i].st=HOVER;}el{b->btns[i].st=NORM;}})}
